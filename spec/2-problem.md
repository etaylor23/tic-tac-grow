...
The brief scales the _size_ but not the _win rule_. Two coherent readings:
...
- **k-in-a-row** (e.g. always 3, or 5 like Gomoku) regardless of N — the classic "m,n,k-game".
  Far more playable on large boards, but needs sliding windows over _every_ diagonal (not just
...
optimisation is _justified_ rather than cargo-culted:
...
3. **Resize timing** — ✅ **Anytime.** Changing size _or_ `k` starts a fresh board via a single
