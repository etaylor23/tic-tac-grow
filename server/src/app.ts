...
app.post("/api/games", async (req, res) => {
  ...
  try {
    ...
    const claimedWinner =
      payload.players.find((p) => p.name === payload.winnerName)?.symbol ??
      null;
  ...
  try {
    const game = await prisma.$transaction(async (tx) => {
      ...
      const xId =
        players[payload.players.findIndex((p) => p.symbol === "X")].id;
      const oId =
        players[payload.players.findIndex((p) => p.symbol === "O")].id;
      const winnerId =
        payload.winnerName === null
          ? null
          : payload.players.find((p) => p.name === payload.winnerName)
                ?.symbol === "X"
            ? xId
            : oId;
    ...
    res.json(
      computeStats(
        ...
        games.map((g) => ({
          winnerId: g.winnerId,
          isDraw: g.isDraw,
          playerIds: g.players.map((gp) => gp.playerId),
        })),
      ),
    );
  } catch {
