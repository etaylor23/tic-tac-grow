...
describe("winnerSymbol", () => {
  ...
  it("finds a k-run on a larger board", () => {
    expect(winnerSymbol(build(5, { 6: "O", 12: "O", 18: "O" }), 5, 3)).toBe(
      "O",
    );
  });
});
...
describe("deriveResult", () => {
  it("reads a decisive game off the move log", () => {
    ...
    expect(deriveResult([0, 3, 1, 4, 2], 3, 3)).toEqual({
      winner: "X",
      isDraw: false,
    });
  });
  ...
  it("reads an O win off the move log", () => {
    ...
    expect(deriveResult([3, 0, 4, 1, 8, 2], 3, 3)).toEqual({
      winner: "O",
      isDraw: false,
    });
  });
  ...
  it("reads a full-board draw", () => {
    expect(deriveResult([0, 1, 2, 4, 3, 5, 7, 6, 8], 3, 3)).toEqual({
      winner: null,
      isDraw: true,
    });
  });
  ...
  it("rejects a log that continues past a win", () => {
    expect(() => deriveResult([0, 3, 1, 4, 2, 5], 3, 3)).toThrow(
      "continues past a win",
    );
  });
});
