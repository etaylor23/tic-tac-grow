...
export const parseGameBody = (body: any): GamePayload => {
  ...
  const parsed: PlayerInput[] = players.map((p: any) => {
    if (!p || typeof p.name !== 'string' || !p.name.trim()) throw new Error('player name required')
    if (p.symbol !== 'X' && p.symbol !== 'O') throw new Error('symbol must be X or O')
    return { name: p.name.trim(), symbol: p.symbol }
  })
  if (parsed[0].symbol === parsed[1].symbol) throw new Error('players need distinct symbols')
  ...
  if (!isInt(winLength, 3, boardSize)) throw new Error('winLength must be 3-boardSize')
