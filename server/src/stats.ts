export type StatGame = { winnerId: number | null, isDraw: boolean, playerIds: number[] }
...
export const computeStats = (players: StatPlayer[], games: StatGame[]): Stat[] =>
  players
    .map(({ id, name }) => {
      ...
      return { name, wins, losses: played.length - wins - draws, draws, played: played.length }
    })
