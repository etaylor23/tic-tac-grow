...
export const Cell = React.memo(
  ({ value, index, isWinning, disabled, onClick }: Props) => (
    <button
      type="button"
      ...
      className={`relative aspect-square w-full overflow-hidden rounded-md ring-1 transition-colors [container-type:inline-size] ${
        isWinning
          ...
          : "bg-white ring-slate-200 enabled:hover:bg-slate-50"
      }`}
    >
      <span
        className={`absolute inset-0 flex items-center justify-center text-[55cqw] font-bold leading-none ${value === "X" ? "text-indigo-600" : "text-rose-500"}`}
      >
        {value}
      </span>
    </button>
