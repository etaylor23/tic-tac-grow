...
export const Main = () => {
  ...
  const canStart =
    !!nameX.trim() && !!nameO.trim() && nameX.trim() !== nameO.trim();
  ...
  const resume = (g: OngoingGame) => {
    ...
    g.moves.forEach((position, i) => {
      board[position] = i % 2 === 0 ? "X" : "O";
    });
    setGame({
      board,
      player: g.moves.length % 2 === 0 ? "X" : "O",
      moves: g.moves,
    });
    setGameId(g.id);
    recorded.current = false;
  ...
  useEffect(() => {
    ...
    saveOngoing({
      id: gameId,
      players: [
        { name: nameX.trim(), symbol: "X" },
        { name: nameO.trim(), symbol: "O" },
      ],
      boardSize: size,
      winLength: k,
      moves: game.moves,
  ...
  return (
    <div className="flex min-h-screen flex-col items-center gap-8 bg-gradient-to-b from-slate-50 to-slate-100 px-4 py-10 text-slate-800">
      <header className="text-center">
        ...
        <p className="mt-1 text-sm text-slate-500">
          Set up a match, play, and track the leaderboard.
        </p>
      </header>
