  ...
  return (
    <div className="flex w-full max-w-7xl flex-col gap-6 xl:grid xl:grid-cols-[minmax(0,2fr)_minmax(0,7fr)_minmax(0,3fr)] xl:items-start">
      ...
      <aside className={`${card} flex min-w-0 flex-col gap-4`}>
        <div
          role="status"
          className={`rounded-lg px-3 py-2 text-center text-lg font-bold ring-1 ${statusTone}`}
        >
          {status}
        </div>
        <div className="flex flex-col gap-1.5">
          {(
            ...
            <div
              key={sym}
              className={`flex items-center gap-2 rounded-lg px-2 py-1.5 ring-1 transition ${!gameOver && player === sym ? "bg-slate-50 ring-slate-300" : "ring-transparent"}`}
            >
              <span
                className={`grid h-6 w-6 place-items-center rounded-md text-sm font-bold text-white ${sym === "X" ? "bg-indigo-600" : "bg-rose-500"}`}
              >
                {sym}
              </span>
        ...
        <p className="text-xs text-slate-400">
          {size}×{size} board · {k} in a row to win
        </p>
