...
beforeEach(() => {
  ...
  fetchMock = jest.fn((url: string) =>
    Promise.resolve({
      ...
      json: () =>
        Promise.resolve(
          String(url).includes("/api/stats") ? statsFixture : { id: 1 },
        ),
    }),
  );
...
const setSlider = (el: HTMLInputElement, value: number) =>
...
const cells = () =>
  screen
    .getAllByRole("button")
    .filter((b) => !ignore.includes(b.textContent || ""));
const postCalls = () =>
  fetchMock.mock.calls.filter((c) => c[0] === "/api/games");
