...
export const Stats = ({ stats }: Props) => {
  if (stats.length === 0) return <p className='text-sm text-slate-400'>No games played yet.</p>
    ...
    <tbody>
      {stats.map(s =>
        <tr key={s.name} className='border-t border-slate-100'>
          ...
          <td className='py-1.5 text-center font-semibold text-emerald-600'>{s.wins}</td>
