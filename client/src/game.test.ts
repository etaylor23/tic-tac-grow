...
describe('winner (3x3, k=3)', () => {
  it('detects a row', () => {
    expect(winner(build(3, { 0: 'X', 1: 'X', 2: 'X' }), 3, 3)).toEqual({ player: 'X', cells: [0, 1, 2] })
  })

  it('detects a column', () => {
    expect(winner(build(3, { 0: 'X', 3: 'X', 6: 'X' }), 3, 3)).toEqual({ player: 'X', cells: [0, 3, 6] })
  })

  it('detects the ↘ diagonal', () => {
    expect(winner(build(3, { 0: 'O', 4: 'O', 8: 'O' }), 3, 3)).toEqual({ player: 'O', cells: [0, 4, 8] })
  })

  it('detects the ↙ diagonal', () => {
    expect(winner(build(3, { 2: 'O', 4: 'O', 6: 'O' }), 3, 3)).toEqual({ player: 'O', cells: [2, 4, 6] })
  })
...
describe('winner (general size/k)', () => {
  it('detects a mid-row horizontal run on a 5x5 with k=3', () => {
    expect(winner(build(5, { 6: 'X', 7: 'X', 8: 'X' }), 5, 3)).toEqual({ player: 'X', cells: [6, 7, 8] })
  })

  it('detects a ↘ diagonal run on a 5x5 with k=4', () => {
    expect(winner(build(5, { 0: 'O', 6: 'O', 12: 'O', 18: 'O' }), 5, 4)).toEqual({ player: 'O', cells: [0, 6, 12, 18] })
  })
...
describe('isDraw', () => {
  it('is true for a full board with no winner', () => {
    const board = build(3, { 0: 'X', 1: 'O', 2: 'X', 3: 'X', 4: 'O', 5: 'O', 6: 'O', 7: 'X', 8: 'X' })
